# Tests — `webship/drupal-patches` (11.4.x)
...
```bash
git fetch --depth=1 https://github.com/webship/drupal-patches.git patches:refs/remotes/upstream/patches
php tests/verify-patch-files.php upstream/patches
```

For every URL in this branch's `composer.json` → `extra.patches."drupal/core"`:
...
A forward-compat placeholder branch that declares no core patches yet passes with nothing to verify.

## 2. Composer Patches applies every core patch
...
[`test.composer.json`](test.composer.json) installs `drupal/core-recommended` at this branch's Drupal core minor, and requires `webship/drupal-patches` from the checkout through a `path` repository. `composer install` therefore applies the branch's curated core patches through the real `cweagans/composer-patches`.
